  ...
  const createHeadRow = () => {
    return (
      ...
      >
        ...
        <th
          key={"commands"}
          style={{ width: props.commandWidth, alignItems: props.alignHead }}
        >
          Commands
        </th>
      </tr>
    ...
    return (
      <tr key={index} className="table-row">
        ...
        {props.displayProps.map((v, i) => (
          ...
          >
            {props.displayConverters[i]
              ? props.displayConverters[i]!(obj[v]).toString()
              : obj[v].toString()}
          </td>
        ))}
        ...
        <td
          key={"commands"}
          id="commands-td"
          style={{ width: props.commandWidth, alignItems: props.alignData }}
        >
          {props.onUpdate ? (
            <button
              type="button"
              className="btn btn-warning command-btn"
              style={{ borderRadius: `${props.borderRadius / 2}px` }}
              onClick={() => {
          ...
          {props.onDelete ? (
            <button
              type="button"
              className="btn btn-danger command-btn"
              style={{ borderRadius: `${props.borderRadius / 2}px` }}
              onClick={() => {
    ...
    <div id="top-div" style={{ borderRadius: `${props.borderRadius}px` }}>
      ...
      >
        <table className="table table-bordered p-0 m-0 table-striped table-dark">
          <tbody className="w-100">{props.data.map(createRow)}</tbody>
        </table>
      </div>
      <div
        key={"table-footer"}
